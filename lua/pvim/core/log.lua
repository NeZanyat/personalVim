...
local logfile = string.format("%s/%s.log", get_cache_dir(), "pvim")
...
function Log:init()
  ...
  local log_level = Log.levels[(pvim.log.level):upper() or "WARN"]
  local pvim_log = {
    pvim = {
      sinks = {
  ...
  local logger = structlog.get_logger "pvim"

  -- Overwrite `vim.notify` to use the logger
  if pvim.log.override_notify then
